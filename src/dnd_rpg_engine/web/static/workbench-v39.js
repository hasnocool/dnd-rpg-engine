...
const AUTHORITY_ROUTE_CONTRACT = [
  ...
  "/characters/", "/npcs", "/distribution/releases", "/distribution/resolve",
];
...
function installWorkshopLinks(){
  const nav=document.querySelector(".app-header nav");
  if(nav&&!nav.querySelector('a[href="/hero"]')){const link=document.createElement("a");link.href="/hero";link.textContent="Hero Creator";nav.prepend(link);}
  const creatorRail=document.querySelector('.rail a[href="/creator"]');
  if(creatorRail&&!document.querySelector('.rail a[href="/hero"]')){const link=document.createElement("a");link.className="rail-link";link.href="/hero";link.innerHTML="<span>★</span> Hero Creator";creatorRail.before(link);}
}
...
function showCommandPalette(){const commands=[["GM Console",()=>setView("gm")],["Session Lobby",()=>openV39View("lobby")],["Tactical Workspace",()=>openV39View("tactical")],["Character Sheet",()=>openV39View("character")],["Create Character / Hero",()=>{location.href="/hero";}],["AI Director",()=>openV39View("director")],["Knowledge Inspector",()=>openV39View("knowledge")],["Analytics",()=>openV39View("analytics")],["Replay",()=>openV39View("replay")],["Automation",()=>openV39View("automation")],["Content Ecosystem",()=>openV39View("content")],["Advance time +10s",()=>tick(10)],["Advance time +1m",()=>tick(60)],["Refresh campaign",()=>refreshCampaign()],["Director quick view",()=>showDirector()]];modal("Command palette",`<label>Filter<input id="palette-filter" autofocus placeholder="Type a command…" /></label><div id="palette-results" class="stack compact"></div>`);const render=()=>{const q=$("palette-filter").value.toLowerCase();const filtered=commands.map((row,index)=>({...row,index})).filter((row)=>row[0].toLowerCase().includes(q));$("palette-results").innerHTML=filtered.map((row)=>`<button data-command-index="${row.index}" class="ghost">${escapeHtml(row[0])}</button>`).join("");document.querySelectorAll("[data-command-index]").forEach((button)=>button.onclick=()=>Promise.resolve(commands[Number(button.dataset.commandIndex)][1]()).catch(reportError));};$("palette-filter").oninput=render;render();}
installWorkshopLinks();
...
$("refresh-current").addEventListener("click",()=>{const loader=viewLoaders[state.activeView];if(loader)loader().catch(reportError);});$("command-palette").onclick=showCommandPalette;document.addEventListener("keydown",(event)=>{if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==="k"){event.preventDefault();showCommandPalette();}});window.addEventListener("focus",()=>{const loader=viewLoaders[state.activeView];if(loader&&state.campaignId)loader().catch(()=>{});});
