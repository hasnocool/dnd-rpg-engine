...
- explicit multiplayer actor ownership checks.
...
For public Internet hosting, deploy behind an authentication/authorization layer and TLS reverse proxy. The built-in `user_id`/`client_id` model is an application authorization primitive, not an identity provider.
